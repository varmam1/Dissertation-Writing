...
Each of the LBP and HOG face descriptors were implemented exactly as mentioned in section \ref{sec:PrepFD} directly without the use of any libraries other than Numpy and OpenCV. However, 
...
WGEML is then saved onto disk, which are the metrics and weights for each face descriptor for each fold. In the end, this is done for each relationship, setting and fold, so for KinFaceW-I, for example, there would be $4 \times 2 \times 5 = 40$ models saved. 
...
the average accuracy is outputted for the given configuration along with each of the individual accuracies, as shown in Figure \ref{fig:OutputOfTesting}. 
