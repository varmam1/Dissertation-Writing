...
Word Count:         & \bf 11999 \\ % TODO: Change
...
Kin recognition is the ability to recognize whether two people are related to each other just by looking at them. 
...
I successfully implemented WGEML and was able to recreate the original accuracies within an acceptable range, thus fulfilling my success criterion. ROC curves were also created in order to have another method of evaluation aside from just using the accuracies. Furthermore, I performed ablation studies in order to determine if there is a relationship between the number of face descriptors used and the accuracy of the model and replaced one face descriptor with a less expensive one.  
...
In humans, specifically, facial resemblance is expected to serve as an indicator of kinship and it has been demonstrated that strangers are able to match photographs of mothers to their infants without any prior contact with any of the family \cite{mateo2015}. 
...
These relationships are conventionally parent-child relationships as opposed to sibling-sibling relationships. A further extension of the main kin verification problem is Tri-Subject Kinship Verification which takes 3 images, two parents and a child, and determines if they are related 
...
In the field of computer vision, there is a goal ``to investigate the bi- or tri- relationship among multiple visual entities'' \cite{TSKinFace} rather than just looking at one entity. 
% Maybe make it to investigate the relationships among multiple images?
...
It was also found that the average accuracy of human performance for the dataset used in the paper was $67.19\%$ and the accuracies ranged from $50\%$ to $90\%$ \cite{fang2010}. As such, the classifiers were doing better than the humans that were tested in the paper. 
...
A widely-used approach for the problem is to use metric learning. One of the first papers that used metric learning was the paper on Neighborhood Repulsed Metric Learning \cite{KinFaceCitation1}. It aims to find a metric that minimizes the distance between the vectors of images that have a kinship relationship and maximize the distances between the vectors of pairs of images that don't have a kinship relationship. 
The Discriminative Multimetric Learning method \cite{DMML} extends this model to use multiple feature vectors instead of just one for each image in the training and testing set.
The paper that will be implemented in this dissertation, Weighted Graph Embedding-Based Metric Learning, is a state-of-the-art metric learning approach to the problem \cite{WGEML}. 

\section{Project Overview}

The project aims to reproduce the results of the paper, Weighted Graph Embedding-Based Metric Learning \cite{WGEML}, or WGEML for short, and to verify the accuracies that were obtained. I also aim to explore the environments the model could work under and infer properties of the model and the datasets used from these environments. 
...
\begin{enumerate}
    ...
    We then explore the consequences of these biases that were found to be present in KinFaceW-II and TSKinFace in section \ref{sec:Biases}.
    \item I further tested the face descriptors by replacing VGG with a smaller CNN in which the implementation is discussed in section \ref{sec:CFNExt} and found that the model does worse but only by $5\%$ at most in section \ref{sec:CFNResults}. 
\end{enumerate}
...
The approach that I'm using requires the use of metric learning, which is discussed in section \ref{sec:MetricLearningPrimer}, as well as the use of multiple face descriptors, which is discussed in section \ref{sec:PrepFD}. I then discuss the core requirements and the extension in \ref{sec:ReqAnal} and the major setup of the project.
...
This differs from face recognition since the task is only to find the faces and not to recognize who the faces belong to. The project uses OpenCV's version of face detection which is a Haar-based cascade classifier and the specifics of which are discussed in Appendix \ref{sec:Cascade}. 
...
for example. Multiple face descriptors can be made for a face image, each of which extracts different features from the face. We use the Local Binary Patterns, Histogram of Gradients, Scale-Invariant Feature Transform, and VGG face descriptors to extract features from each face.
...
Another face descriptor is the Histogram of Gradients (HOG) \cite{HOG}. Given a colored image, we can think of the image as a function, $I: \mathbb{N}_m \times \mathbb{N}_n \to \mathbb{R}^3$, which takes a coordinate in the image and returns a vector of values, which correspond to the red, green, and blue values of the pixel.
...
First, we need to calculate the gradients of the image. As the image function is discrete, so we cannot analytically find the gradients, we must find approximations to do so. This is done by convolving specific kernels on the image. 
...
we define the gradient of each pixel to be the gradient vector that has the maximum magnitude among the 3 channels. 
...
There are 3 main stages to running the project, which are, for each dataset, the preprocessing stage, the training stage, and the testing stage. The important outputs of each of these stages are saved onto disk so that each stage doesn't have to be run right after another. 
...
\begin{figure}[h!]
    ...
    \caption{The mean ROC curve for each relationship averaged over each fold, dataset and setting}
    \label{fig:ROCRel}
\end{figure}
...
In order to look at potential biases that could occur in each of the datasets, each model was tested on all of the available datasets. So, for example, the model that was returned from training on the KinFaceW-I unrestricted dataset for the Father-Son relationship will have been tested on all of the 3 datasets that were used. 
...
found that the FS relationship had the least change in accuracy with an average 0.092\% 
...
To make sure the project was doing everything that it was supposed to be doing, unit tests were created for each file except the scripts. This is since the scripts were for integrating the modules together and thus didn't require unit testing.
...
Overall, the project was successful since I was able to implement each of the core components, as specified in section \ref{sec:ReqAnal} and, this allowed me to replicate the findings of the WGEML paper within a 15\% error range. 
...
In the future, the model can be extended to work on videos. This was an extension at the beginning of the project which ended up not coming to fruition since I couldn't obtain a dataset for it.
...
Furthermore, in the original results, there wasn't much difference in accuracies between the unrestricted and restricted settings in many of the cases. In this case, the effect the negative pairs have on the algorithm can be further explored and whether different sets of negative pairs can give statistically significant differences in the accuracies. 
