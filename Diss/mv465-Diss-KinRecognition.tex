...
SIFT \cite{SIFT} is another way of obtaining face descriptors, although the original SIFT algorithm differs from how it is used in the project, though it is still important to briefly go over. The algorithm, generally, is split into finding the keypoints, fine tuning the keypoints, assigning an orientation to each keypoint and then getting a descriptor from each keypoint. 
...
From here, for each octave, a difference of Gaussians is created to help find the keypoints of the image, which are just points of interest which can be near important features of the image. The difference of Gaussians is just the difference between the consecutive Gaussians. This can be visualized in figure \ref{fig:DiffOfGauss}.
...
K-Nearest Neighbors is an algorithm that finds the $K$ nearest neighbors for each datum in a given set of data. Given a list of datapoints in Euclidean space, we wish to find, for each datapoint, the $K$ closest datapoints to it. The \texttt{sklearn} library is used to implement it which either uses a brute force algorithm, a ball tree or a $kd$-tree, which is discussed in appendix \ref{sec:KNNAlgos}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.7]{Eval/OrigResults.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.7]{Eval/DiffOrigResults.png}
    \caption{Differences in accuracies between my implementation and \cite{WGEML} in \%}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.6]{Eval/KFWII-Unrestricted-AbStudy.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.6]{Eval/KFWI-Unrestricted-AbStudy.png}
...
\section{K-Nearest Neighbor Algorithms} \label{sec:KNNAlgos}
