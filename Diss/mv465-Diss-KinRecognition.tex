...
To talk about Convolutional Neural Networks, first we must discuss what a neural network is. 
...
A Convolutional Neural Network (CNN) is a type of neural network which is used to process data with a grid pattern such as images. As opposed to an artificial neural network which is just composed of fully-connected layers, a CNN also uses convolutional layers and pooling layers \cite{CNN1}. 
...
This differs from face recognition since the task is only to find the faces and not to recognize who the faces belong to. 
...
In the CifarNet extension, I replaced the VGG network with a smaller network that was trained on CIFAR-10 \cite{CIFAR10}, which is called CifarNet\footnote{Architecture taken from https://github.com/deep-fry/mayo/blob/master/models/cifarnet.yaml}, until it was able to achieve a 91\% accuracy with it. The architecture is shown in figure \ref{fig:CFNArch}.
...
When only positive pairs were used for the TSKinFace dataset for both training and testing, we get accuracies that are fairly close to that of the original paper with an average difference of $0.742\%$. However, when negative pairs were added to the test set, the accuracies drop by, on average, $8.29\%$. As a note, in the later sections of this chapter, TSKinFace refers to having all pairs in both the training and test set. 
...
In order to look at potential biases that could occur in each of the datasets, each model was tested on all of the datasets that were available. So, for example, the model that was returned from training on the KinFaceW-I unrestricted dataset for the Father-Son relationship will have been tested on by all of the 3 datasets that were used. 

In every case, the accuracies went down when the test set that was used came from a different dataset than the training set. This is to be expected as the model might be generalizing on small biases that are inherent in every dataset. 
...
However, the amount the accuracy increases by decreases as you increase the number of face descriptors (ie. the second derivative is negative). An example of this is shown with KinFaceW-II unrestricted which is shown in figure \ref{fig:KFW-IIUAbStudy}.
...
accuracy difference. This is shown in figure \ref{fig:CFNRelationshipGrouping}.
...
Where a positive number implies that the original face descriptors did better. There is more variation in the other relationships, 
ignoring the tri-kin relationships as there is only one dataset for this, than there is in the father-son relationship, 
and this is reflected in the standard deviations for each relationship. 
...
The coverage for each file and overall are shown in figure \ref{fig:Coverage}.
...
Overall, the project was successful since I was able to replicate the findings of the WGEML paper within a 15\% error range. 
This meant that I was able to implement each of the core components, as specified in section \ref{sec:ReqAnal}. Furthermore, I was able to further explore the intricacies of the algorithm and how it reacts to variations in the input. By doing ablation studies, 
...
% I was able to implement the WGEML approach to kin recognition and further explore the 
