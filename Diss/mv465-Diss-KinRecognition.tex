...
In this chapter, I discuss the necessary technical background information needed for the project in sections \ref{sec:CNN} through \ref{sec:PCA}, then the requirements of the project are elaborated on in section \ref{sec:ReqAnal} before the starting point and Software Engineering practices are discussed in section \ref{sec:SEPractices}. 

\section{Convolutional Neural Networks} \label{sec:CNN}

% TODO: what is a neural network
To talk about Convolutional Neural Networks, first we must discuss what an artificial neural network (ANN) is. This is just a set of layers like we see in figure \ref{fig:ANNEx} which have neurons in each of the layers which each output one value. 
...
If we have that $\mathbf{x}$ is the input layer, $\mathbf{h_i}$ is the $i$th hidden layer and $\mathbf{y}$ is the output layer, then an ANN can be written as:
...
A kernel is a matrix which is usually small and has sizes less than that of the input image but expands the depth of the entire input image. So, for example, if the input to a convolutional layer was $32 \times 32 \times 64$, then a kernel could have size $3 \times 3 \times 64$ where the first two sizes are specified. 
The kernel is then started in the top left of the input and the dot product of the kernel and the input sub-array is taken to be the output at that cell. Then the kernel is moved along as much as specified in the stride. So if the stride was $2 \times 2$, then the kernel would move 2 to the right and do the same thing. An illustration of this is seen in figure \ref{fig:ConvLayer}.
...
A bias is then applied and the activation function, which is usually ReLU, is applied to the output. 

\subsection{Pooling Layers}

Pooling layers in a CNN tend to reduce the dimension of the representation. This then reduces the number of parameters in the representation. 
There are multiple pooling layers, such as max-pooling and average-pooling, which are the two types that are used in this project in VGG and CifarNet which are discussed in sections \ref{sec:VGGPrep} and \ref{sec:CFNExt}. 
...
There are a multitude of functions which are applied to the output of a layer which have different effects. These are called activation functions and we'll talk about two of them here, ReLU and Softmax. 
...
In otherwords, it makes sure that all negative values become 0. This is simple to calculate and has the property that the derivative is either 1 or 0 which makes the gradient computation simpler. Due to the simplicity of the activation function, this helps speed up training a neural network if it is used as the activation layer for some layers as opposed to other ones. 

\subsubsection{Softmax}

Another one if the softmax function which takes a vector, $\mathbf{z}$ and the component-wise output is:

\[\sigma(\mathbf{z})_i = \frac{e^{z_i}}{\sum_{j = 1}^{N} e^{z_j}}\]

Where $N$ is the dimension of the input vector. In other words, it takes a vector and outputs a vector of the same size where the sum of the values of the vector add up to 1 so it makes it so you can interpret the individual values as probabilities. This is a useful activation function for the last layer of a neural network that solves a classification problem. For example, in the CifarNet model in section \ref{sec:CFNExt}, the last layer has the softmax function applied to it in order for each of the 10 values to be interpreted as a probability that the input image was the corresponding class. 

\section{Face Detection} \label{sec:FaceDetection}

Face detection is the task of finding a face within a given image and returning the set of faces found. 
%  as seen in Figure \ref{fig:FaceDetEx}. 
This differs from face recognition since the task is only to find the faces and not to recognize who the faces belong to. The project uses OpenCV's version of face detection which is a cascade classifier and is talked about in appendix \ref{sec:Cascade}. 
...
We wish to be able to create a mapping from a colored image into a vector to make computations easier and to be able to determine similarity between images, which we do using the cosine similarity in this project as seen in section \ref{sec:PredictionModule}. These are called image descriptors for general images. When we try and map face images to vectors, we call these face descriptors. 
...
This makes it so that we only have to learn what this linear transformation is which is simpler to find than searching through the set of all possible distance functions to see which is the best overall distance function. 
...
\begin{figure}[h!]
    \dirtree{%
        ...
        .4 SIFT.py.
        .4 VGG.py.
        .3 face\_detection/face\_detection.py.
...
We split the datasets for each relationship into 5 folds each in which each fold within a relationship has around the same number of pairs. 
...
randomly. For each relationship, the number of negative pairs was set to be equal to the number of positive pairs in the corresponding set, which was 404 for the training set and 101 for the test set. Then, two images were picked out randomly such that they didn't belong to the same positive pair.
...
%  as shown in figure \ref{fig:Patches}.
...
We also see that the Father-Son relationship does better than the other relationships for the same dataset except in the Mother-Son relationship for TSKinFace when there were only positive pairs in the training set and all pairs in the testing set. 
...
Furthermore, referring to the requirements analysis in section \ref{sec:ReqAnal}, each of the core project requirements were completed, which were to implement face detection, the necessary face descriptors, WGEML, a prediction method and replicate the paper end-to-end. Furthermore, the extensions were also completed and the results of which are discussed in sections \ref{sec:Biases}, \ref{sec:AbStudies}, and \ref{sec:CFNResults}.
...
However, as you increase the number of face descriptors, there are diminishing returns on the accuracy (ie. the second derivative is negative). An example of this is shown with KinFaceW-II unrestricted which is shown in figure \ref{fig:KFW-IIUAbStudy}.
% Could group the two ab study plots in one figure?
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.75]{Eval/KFWII-Unrestricted-AbStudy.png}
    \caption{The plot of accuracy versus number of face descriptors used for KinFaceW-II Unrestricted}
    \label{fig:KFW-IIUAbStudy}
\end{figure}
...
increases it more, as seen in figure \ref{fig:KFW-IUAbStudy}.

\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.75]{Eval/KFWI-Unrestricted-AbStudy.png}
    \caption{The plot of accuracy versus number of face descriptors used for KinFaceW-I Unrestricted}
    \label{fig:KFW-IUAbStudy}
\end{figure}
...
% TODO: Might want to put the standard deviations here as a graph or a table?
...
This means that using the CFN model can be useful in mobile and embedded systems whereas having the extra computation and size cost might not be worth it for the $5\%$ gain in some cases. However, the CFN model does need to be trained whereas, with the VGG model, the weights are just obtained from the website of the authors of the paper describing how to use VGG for faces \cite{VGGFace}. Thus, training time is essentially 0 for VGG but that doesn't help the fact that running the model will still, inevitably, take more computation time than CFN. 
 
% With respect to why KinFaceW-II did better with the CFN face descriptor, I'm not entirely sure but I think it 
% goes back to the fact that the pairs of images are taken from the same image in KFW-II but not in KFW-I. 
% My guess is that, since the input to the CFN network is a 32 x 32 image, potentially it could be 
% generalizing the lighting and other non-face related features more which could be exacerbating 
% the problem we saw earlier with the bias. The main reason I'm unsure about this is since I 
% would've imagined something similar would have occurred with TSKinFace if this was the case 
% but it doesn't seem to have happened, or at least not with the same magnitude of effect. 
...
% basic research skills (vagueness of papers + how to learn from online)
% Dependencies
...
% Could also look into how a proper SIFT implementation would affect things. 

% Using WGEML on different datasets which dont have the bias that KFW-II has, 
% Creating a proper application for it?
...
\chapter{Cascade Classifier} \label{sec:Cascade}
